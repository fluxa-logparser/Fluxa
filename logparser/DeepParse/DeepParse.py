...
def sensitivesampling(kernel_matrix, max_length, epsilon=1E-10):
...
def getSensitivesamplingIndex(log_emb_list, 
    ...
    t = time.time()
    result = sensitivesampling(similarities, max_length)
    result.sort()
    print('sensitivesampling algorithm running time: ' + '\t' + "{0:.4e}".format(time.time() - t))
    return result


def sensitivesamplingsplit(log_list, groundtruth_template, candidate_idx):
...
def evaluatePA(groundtruth, result):
    ...
    for i in range(length):
        ...
        '''
                    ...
                    print(count)
                    print(result['template'][i])
                    print(groundtruth.loc[groundtruth['Content'] == result['log'][i]]['EventTemplate'].values[0])
        '''
...
def evaluateGA(groundtruth, result):
    ...
    for parsed_group_list in result_dict.values():
        ...
        if flag == 0:
            ...
            print("```````````")
            print(parsed_group_list)
            print(len(parsed_group_list))
            print("Group length")
            print(gt_group_list)
            print(len(gt_group_list))
            print("Length in correct group")
...
class ModelParser():
  def __init__(self, 
        ...
        emb_path,
        cand_ratio,
        split_method, # random or sensitivesampling
  ...
  def splitCandidates(self, groundtruth_path, cand_ratio, method="random"):
      ...
      if method == "random":
          ...
          candidate_idx = getSensitivesamplingIndex(log_embs, 2000, cand_ratio)
          log_test, log_cand, gt_test, gt_cand = sensitivesamplingsplit(log_list, groundtruth_template, candidate_idx)
  ...
  def generateEmbeddings(self, str_list):
      ...
      # Note: You need to set openai.api_key before calling this method
      # Example: openai.api_key = "your-api-key-here"
      # Or set it as environment variable: export OPENAI_API_KEY="your-api-key-here"

      if not openai.api_key:
          raise ValueError("OpenAI API key not set. Please set openai.api_key before calling generateEmbeddings()")

      return [openai.Embedding.create(log,model="text-embedding-ada-002") for log in str_list]
  ...
  def generateLuMap(self, look_up_map_path):
      ...
      for test_idx in tqdm(range(len(self.log_test))):
        ...
        for cand_idx in range(len(self.log_cand)):
          dis_dict[self.cosine_similarity(test_embs[test_idx], cand_embs[cand_idx])] = cand_idx
  ...
  def cosine_similarity(self,vector1, vector2):
      ...
      Returns:
      float: Cosine similarity value, ranging from [-1, 1]
      """
...
and put the template after <extraction> tag and between <START> and <END> tags."
      ...
      if self.evaluate:          
        ...
        df.to_csv("DeepParse_bechmark_result.csv", index=False, float_format="%.6f")
      return 